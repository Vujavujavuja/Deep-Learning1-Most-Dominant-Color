...
class ColorDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        target_in_dataset = eval(row["Dominant Color"])

        if self.transform:
            image = self.transform(image)

        target_in_dataset = torch.tensor(target_in_dataset, dtype=torch.float32) / 255.0
        return image, target_in_dataset
...
if __name__ == "__main__":
    ...
    print("Model saved as resnet_v2.pth")
